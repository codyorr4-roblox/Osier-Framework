...
local playerRemovingEvents, playerRemovingCount = {},0
local leftEarly = {}
	...
	if(success)then
		if(loadedData)then
			if(loadedData._OsierSessionId == "") then
				...
				player:Kick("Another session is still being saved, could not load data.")
			end
		else
...
local function update(player, leaving)
	...
	local success, err = pcall(function()
		store:UpdateAsync("PlayerSaveData", function(oldData)
			...
			if(oldData and cachedData)then
				local sessionId = cachedData._OsierSessionId
				
				if(sessionId == game.JobId)then
					if(leaving)then cachedData._OsierSessionId = "" end
					return cachedData
				elseif(sessionId == nil or sessionId == "")then
					cachedData._OsierSessionId = (leaving==true) and "" or game.JobId
					return cachedData
				else
	...
	if(success)then
		...
		player:Kick("Could not save your data, contact a developer.")
	end
end

function startAutoSaving()
	coroutine.wrap(function()
	    local i = (rs:IsStudio() or server.SaveInterval<=15) and 15 or server.SaveInterval
		local t = os.time() + i
		print("Started autosaving at "..i.." second intervals")
		
		while(canStart==false)do
...
function handleRemotes(player)
	...
	dataReplicator:FireClient(player, server:GetData(player))
end
...
function server:Start(newDefaultData)
	...
	-- changes the default data table
	defaultData = newDefaultData
	
	-- starts autosaving...
	startAutoSaving()
	
	-- tells the server when a client is ready.
	...
	ps.PlayerAdded:Connect(function(player)
		script.Parent.Client:Clone().Parent = player:WaitForChild("PlayerGui")

		-- handles respawning
		player.CharacterAdded:Connect(function(character)
			if(clientsReady[uid(player)])then
				rs.Stepped:Wait()
				script.Parent.Client:Clone().Parent = player:WaitForChild("PlayerGui")
				player.DataReplicator:FireClient(player, server:GetData(player))
			end
		end)
		...
		-- fire all custom player added events when data/remotes are ready.
	...
	ps.PlayerRemoving:Connect(function(player)
		...
		-- fire all custom player removing events before data/remotes are destroyed
		for _, playerRemoving in pairs(playerRemovingEvents)do
			coroutine.wrap(function()
				if(type(playerRemoving) == "function")then
...
function server.PlayerRemoving:Connect(f)
	local id = playerRemovingCount+1
	playerRemovingCount+=1
	playerRemovingEvents["PlayerRemoving"..id] = f
	local connection = {}
	function connection:Disconnect()
		playerRemovingCount-=1
		playerRemovingEvents["PlayerRemoving"..id]=nil
	end
	return connection
end
