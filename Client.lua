...
function start()
	...
	local event = client.Player:WaitForChild("OsierEvent", 24)
	local request = client.Player:WaitForChild("OsierRequest", 24)
...
function client:WaitForStart()
	...
	repeat
		if(os.time() >= start)then
			client.Player:Kick("Could not start the client, contact a developer and try again.")
		end
		rs.Stepped:Wait()
