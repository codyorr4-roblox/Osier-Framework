...
local function load(player)
	...
	if(success)then
		if(loadedData)then
			if(loadedData._SlimSessionId == "") then
		...
		sessionData[id]._SlimSessionId = game.JobId
...
local function update(player, leaving)
	...
	if(leaving)then
		cachedData._SlimSessionId = ""
	end
	
	local success, err = pcall(function()
		store:UpdateAsync("PlayerSaveData", function(oldData)
			if(oldData)then
				if(cachedData and cachedData._SlimSessionId == game.JobId)then
					return cachedData
				else
...
function handleRemotes(player)
	...
	event.Name = "SlimEvent"
	event.Parent = player
	
	local request = Instance.new("RemoteFunction")
	request.Name = "SlimRequest"
...
function server:Start(newDefaultData)
	...
	server:HandleEvent("_SlimClientReady", 5, function(player,data)
...
function server:WaitForClient(player)
	local id = uid(player)
	if(clientsReady[id] and sessionData[id] and player:FindFirstChild("SlimEvent") and player:FindFirstChild("SlimRequest") and player:FindFirstChild("DataReplicator"))then
		return
	end
	...
	repeat
		...
		rs.Stepped:Wait()
	until clientsReady[id] and sessionData[id]~=nil and player:FindFirstChild("SlimEvent") and player:FindFirstChild("SlimRequest") and player:FindFirstChild("DataReplicator")
...
function server:Fire(player, name, data)
	...
	local event = player:WaitForChild("SlimEvent", 12)
	if(event)then
